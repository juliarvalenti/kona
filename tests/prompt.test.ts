...
test("the launcher advertises both filter and copy-prompt, on keys that can coexist", async () => {
  // The launcher starts a filter on any printable key (#40), so copy-prompt
  // can't have a bare letter there — it takes the ctrl chord of the same letter
  // instead (#62), which `startsFilter` never claims.
  const frame = await renderLauncher([demo, other], 0, 80, 12);
  expect(frame).toContain("filter");
  expect(frame).toContain("ctrl+y prompt");
  // Still not the bare letter: typing `y` on the launcher narrows the list.
